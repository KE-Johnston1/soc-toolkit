...
def assess_risk(case: RiskInput) -> RiskAssessment:
    ...
    gaps: list[str] = []
    for name, value in (("asset criticality", case.asset_criticality), ("account privilege", case.account_privilege), ("data sensitivity", case.data_sensitivity), ("likelihood", case.likelihood), ("business impact", case.business_impact), ("financial impact", case.financial_impact)):
        if value == "Unknown":
    ...
    known_high = sum(value == "High" for value in (case.asset_criticality, case.account_privilege, case.data_sensitivity, case.likelihood, case.business_impact))
    if known_high >= 3:
        overall = "High"
    elif known_high >= 1:
        overall = "Medium"
        ...
        overall = "Low"
    rationale = ("Risk context is provisional because one or more impact, likelihood, or financial inputs remain unknown. Risk does not establish compromise." if gaps else "Risk is derived from the supplied organisational context; it is not a statement that the underlying security event is confirmed.")
    return RiskAssessment(overall, case.confidence, rationale, tuple(gaps), case.financial_basis, case.legal_regulatory_consideration, case.cve_relevance, case.cvss_severity)
